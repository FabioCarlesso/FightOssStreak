...
4. **Login social com acesso sob aprovação** (D36, que revisitou a D9). Conta nasce `PENDENTE` e quem libera é o autor (`fos.auth.owner-emails`). Sem sessão é 401; sem liberação é 403 com o motivo. O usuário da requisição sai do `CurrentUserProvider` — nunca de uma consulta que cria usuário quando não acha ninguém. Provedor sem credencial não é registrado: a aplicação sobe sem segredo nenhum, e é assim que dev e CI rodam. Papéis, admin genérico e cadastro aberto seguem fora de escopo.
